...
function Auth({ onAuthSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onAuthSuccess(userCredential.user, idToken);
    } catch (error) {
      ...
      <div className="bg-gray-800 p-8 rounded-xl border border-gray-700 max-w-md w-full mx-4">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-400">
            {isLogin ? 'Sign in to join the chat' : 'Create an account to get started'}
          </p>
        </div>

        {error && (
          <div className="bg-red-800 border border-red-600 text-red-200 p-3 rounded-lg mb-4 text-sm">
            {error}
          </div>
        )}
        ...
        <div className="mt-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full mt-4 bg-white hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed text-gray-900 py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
          >
